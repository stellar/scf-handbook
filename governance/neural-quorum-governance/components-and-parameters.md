...
* [Tier](components-and-parameters.md#id-1.1.2.1.-verified-tier-and-discord-roles-neuron)
...
1. Instead of calculating PageRank and Highly Trusted Bonus only for the current round, we run it for current and previous rounds.
2. Then for each user calculate the % difference between the two results. Take those 3 examples:
   1. In round #32 user “A” has trust score of 1.0 and in #33 it grew to 1.2 because more people trusted him. This means his current trust is equal to 120% of previous trust.
   2. In round #32 user “B” has trust score of 1.0 and in #33 it fell to 0.8 because a few less people trusted him. This means his current trust is equal to 80% of previous trust.
...
$$
D = \frac{T_c}{T_p} \cdot 100
$$

$$
M = 30 + \frac{(100 - 30)}{\left(1 + 1 \cdot e^{-0.2 \cdot (D - 70)}\right)^{1/3}}
$$
...
<sub>Where: <i>D</i> = what % of previous trust is current trust, <i>T<sub>c</sub></i> = current trust (page rank + HTB from current round), <i>T<sub>p</sub></i> = previous trust (page rank + HTB from previous round), <i>M</i> = outcome of Generalized Logistic Function with <i>D</i> as an input, and some arbitrary parameters, <i>T<sub>f</sub></i> = final trust score</sub>&#x20;
...
<table><thead><tr><th>Parameter</th><th width="355.34375">Description</th><th>#</th></tr></thead><tbody><tr><td>Min Quorum Candidates</td><td>Minimum size of potential candidates a user must select for their Quorum</td><td>6</td></tr><tr><td>Max Quorum Candidates</td><td>Maximum size of potential candidates a user can select for their Quorum</td><td>15</td></tr><tr><td>Quorum Participation Threshold</td><td>Minimum % of quorum participants needed for a valid vote</td><td>5</td></tr><tr><td>Quorum Size</td><td>Max number of candidates considered in a Quorum Vote</td><td>15</td></tr><tr><td>Relative Agreement Threshold</td><td>Minimum % of agreement required among active votes</td><td>51%-67%*</td></tr></tbody></table>
...
3. Since the quorum meets the consensus threshold (51%), Alice's vote (with her own voting power assigned through Neural Governance) is counted as "Yes."
{% endhint %}
