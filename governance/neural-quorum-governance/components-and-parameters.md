...
<sub>Where: $D$ = what % of previous trust is current trust, $T_c$ = current trust (page rank + HTB from current round), $T_p$ = previous trust (page rank + HTB from previous round), $M$ = outcome of Generalized Logistic Function with $D$ as an input, and some arbitrary parameters, $T_f$ = final trust score</sub>&#x20;
